...
class HomeViewTest(TestCase):
    ...
    def test_home_page_all_posts(self):
        ...
        posts = [post.__repr__() for post in Post.objects.all().order_by('-pub_date')]
    ...
    def test_user_posts_page_only_users_posts(self):
        # /blog/john_lennon
        url = reverse('blog:user_posts', kwargs={'username': self.test_user.username})
        response = self.client.get(url)
        posts = [post.__repr__()
                 for post
                 in Post.objects.filter(user=self.test_user).order_by('-pub_date')]
        self.assertQuerysetEqual(response.context['posts'], posts)
        self.assertContains(response, self.test_user.first_name)
        self.assertContains(response, self.test_user.last_name)
        self.assertEqual(response.context['first_name'], self.test_user.first_name)
        self.assertEqual(response.context['last_name'], self.test_user.last_name)
